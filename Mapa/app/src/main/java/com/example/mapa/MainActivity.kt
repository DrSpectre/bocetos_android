...
class MainActivity : ComponentActivity() {
    private lateinit var gps_sensor: ClienteGPS
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContent {
            val tenemos_permitido_usar_gp: Boolean by controlador_gps.autorizacion.observeAsState(initial = false)

            LaunchedEffect(tenemos_permitido_usar_gp) {
                if(tenemos_permitido_usar_gp){
    ...
    companion object{
        const val permisos_totales_gps = 123
    }
