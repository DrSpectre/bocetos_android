...
class ProveedorDePistas( ) {
    ...
    fun ubicar_pista_mas_cercana(ubicacion: Location): InformacionPista{
        ...
        var pista_mas_cercana: InformacionPista = arreglo_de_pistas[0]
        var distancia_pista_mas_cercana: Float = 9999999.9f

        for(pista in arreglo_de_pistas){
            val distancia_calculada = pista.ubicacion.calcular_distancia(ubicacion)
            if (distancia_pista_mas_cercana > distancia_calculada){
    ...
    private fun _generar_pistas_actuales(){
        ...
        nueva_pista = InformacionPista(
            ...
            "Foto de una palanca de Gordon freeman",
